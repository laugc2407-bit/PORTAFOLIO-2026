...
def proyecto(titulo, resumen, archivo="", galeria=None, rol="", herramientas="", resultado="", enlace=""):
    return {
        ...
        "galeria": galeria or [],      # o VARIAS imágenes/videos/Vimeo (carrusel)
...
# Ancho máximo del contenido dentro de cada sección. En pantallas más
# anchas que esto, el padding izquierdo/derecho crece de forma simétrica
# (ver calc() en inject_css) para que el contenido quede centrado en vez
# de pegado a la izquierda con un enorme espacio vacío a la derecha.
CONTENT_MAX = 1400
...
def inject_css():
    st.markdown(
        ...
        /* ---------- generic section shells, one per "temperature" step -----
           ...
           The horizontal padding uses calc() so the CONTENT_MAX-px content
           column stays centered on wide screens: past that width extra
           space is pushed equally into left/right padding instead of
           piling up on one side, while the background still spans the
        ...
        .st-key-interfaces, .st-key-visual, .st-key-research, .st-key-contact {{
            ...
            padding-left: max(60px, calc((100% - {CONTENT_MAX}px) / 2)) !important;
            padding-right: max(60px, calc((100% - {CONTENT_MAX}px) / 2)) !important;
            position: relative;
            overflow: visible;
            border-bottom: 3px solid var(--ink);
        }}
...
def _vimeo_id(ref: str):
    """Si <ref> es una referencia a un video de Vimeo, devuelve su ID.
    Acepta cualquiera de estas formas dentro de una 'galeria':
      - "vimeo:824804226"
      - "https://vimeo.com/824804226"
      - "https://player.vimeo.com/video/824804226"
    Si no es una referencia de Vimeo, devuelve None (así el resto del
    código sabe que debe tratarlo como archivo local)."""
    if not isinstance(ref, str):
        return None
    if ref.startswith("vimeo:"):
        digits = ref.split(":", 1)[1].strip()
        return digits if digits.isdigit() else None
    match = re.search(r"vimeo\.com/(?:video/)?(\d+)", ref)
...
def carousel_widget(media_list, label, height=280):
    """Carrusel de fotos/video/Vimeo con altura FIJA. Al no depender de
    ningún resize dinámico (a diferencia de un acordeón), es la parte que
    se puede meter en un iframe sin que se corte ni se descuadre.

    Cada elemento de media_list puede ser:
      - una imagen local:      "foto.jpg"
      - un video local:        "clip.mp4"
      - un video de Vimeo:     "vimeo:824804226"
                          o     "https://vimeo.com/824804226"
    Se pueden mezclar todos en la misma galería sin problema."""
    if not media_list:
    ...
    for i, rel in enumerate(media_list):
        vimeo_id = _vimeo_id(rel)
        if vimeo_id:
            has_vimeo = True
            src = (
                f"https://player.vimeo.com/video/{vimeo_id}"
                "?title=0&byline=0&portrait=0&background=0&autopause=0"
                "&muted=1&loop=1&playsinline=1"
            )
            slides_html.append(
                f'<div class="hm-slide hm-slide-vimeo" data-slide-index="{i}">'
        ...
        if kind == "video":
            ...
            slides_html.append(
                f'<div class="hm-slide" data-slide-index="{i}">'
                f'<video src="{uri}" {autoplay}muted loop playsinline></video></div>'
            )
        else:
            slides_html.append(
                f'<div class="hm-slide" data-slide-index="{i}"><img src="{uri}" alt="{label}"></div>'
            )
    ...
    vimeo_script = '<script src="https://player.vimeo.com/api/player.js"></script>' if has_vimeo else ""
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .hm-slide {{ flex: 0 0 100%; height: 100%; position: relative; }}
        .hm-slide img, .hm-slide video {{ width: 100%; height: 100%; object-fit: cover; display: block; }}
        .hm-slide-vimeo iframe {{ width: 100%; height: 100%; display: block; }}
        .hm-arrow {{
    ...
    {vimeo_script}
    <script>
        const hmVimeoPlayers = {{}};

        function hmInitVimeo(root) {{
            if (typeof Vimeo === 'undefined') {{
                setTimeout(() => hmInitVimeo(root), 150);
                return;
            }}
            root.querySelectorAll('.hm-slide-vimeo iframe').forEach((iframe) => {{
                const i = parseInt(iframe.closest('.hm-slide').dataset.slideIndex);
                const player = new Vimeo.Player(iframe);
                player.setVolume(0).catch(() => {{}});
                hmVimeoPlayers[i] = player;
            }});
            hmSetIndex(root, parseInt(root.dataset.index || '0'));
        }}
        ...
        function hmSetIndex(root, idx) {{
            ...
            slides.forEach((s, i) => {{
                ...
                const vp = hmVimeoPlayers[i];
                if (vp) {{
                    if (i === idx) {{ vp.play().catch(() => {{}}); }}
                    else {{ vp.pause().catch(() => {{}}); }}
                }}
            }});
        ...
        {"hmInitVimeo(document.querySelector('.hm-carousel'));" if has_vimeo else ""}
    </script>
    """
...
def section_hero():
    with st.container(key="hero"):
        ...
        st.markdown(
            f"""
            <div style="position:relative; padding:70px max(60px, calc((100% - {CONTENT_MAX}px) / 2)) 80px max(60px, calc((100% - {CONTENT_MAX}px) / 2)); background:var(--void); color:var(--cream); overflow:hidden;">
...
def render_missing_assets_banner():
    ...
    missing = [p for p in expected if p and not _vimeo_id(p) and not (ASSETS / p).exists()]
    if missing:
...
def section_contact():
    with st.container(key="contact"):
        ...
        st.markdown(
            f"""
            <div style="position:relative; padding:60px max(60px, calc((100% - {CONTENT_MAX}px) / 2)) 80px max(60px, calc((100% - {CONTENT_MAX}px) / 2)); background:var(--void); color:var(--cream); overflow:hidden;">
